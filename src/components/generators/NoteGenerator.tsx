...
const NoteGenerator = () => {
  ...
  const [formData, setFormData] = useState(() => {
    ...
    return {
      ...
      sourceLessonContext: undefined as {
        keyWords?: string;
        lessonFocus?: string;
        tlrs?: string;
  ...
  useEffect(() => {
    if (location.state?.preloaded && !hasAutoGeneratedRef.current) {
      const p = location.state.preloaded;
      const isAuto = !!location.state.autoGenerate;

      const loadedFormData = {
        level: p.level || formData.level,
        class: p.class || formData.class,
        subject: p.subject || formData.subject,
        ghanaianLanguage: p.ghanaianLanguage || formData.ghanaianLanguage,
        strand: p.strand || formData.strand,
        subStrand: p.subStrand || formData.subStrand,
        contentStandard: p.contentStandard || formData.contentStandard,
        indicator: p.indicator || formData.indicator,
        lessonTopic: p.lessonTopic || formData.lessonTopic || '',
        objectives: p.objectives || (p.indicator ? formatPerformanceIndicator(p.indicator) : formData.objectives),
        duration: p.duration || formData.duration,
        week: p.week || formData.week,
        term: p.term || formData.term,
        academicYear: p.academicYear || formData.academicYear,
        locality: p.locality || formData.locality,
        specificLocality: p.specificLocality || formData.specificLocality,
        differentiation: p.differentiation || formData.differentiation,
        language: p.language || formData.language,
        bilingualLanguage: p.bilingualLanguage || formData.bilingualLanguage,
        coreCompetencies: p.coreCompetencies || formData.coreCompetencies,
        sourceLessonContext: p.sourceLessonContext || formData.sourceLessonContext
      };
      ...
      if (isAuto) {
        ...
        if (loadedFormData.contentStandard && loadedFormData.class) {
          if (!matchStandardToClass(loadedFormData.contentStandard, loadedFormData.class, loadedFormData.level)) {
            toast.error(`The selected curriculum item does not match ${loadedFormData.class}. Please select a valid curriculum standard.`);
            setStep(3);
            return;
          }
        }
        if (loadedFormData.indicator && loadedFormData.class) {
          if (!matchStandardToClass(loadedFormData.indicator, loadedFormData.class, loadedFormData.level)) {
            toast.error(`The selected indicator does not match ${loadedFormData.class}. Please select a valid curriculum indicator.`);
            setStep(3);
            return;
          }
        }
        ...
        const currentDisplaySubject = (loadedFormData.subject === 'Ghanaian Language' && loadedFormData.ghanaianLanguage)
          ? `Ghanaian Language (${loadedFormData.ghanaianLanguage})`
          : loadedFormData.subject;

        setLoading(true);
        toast.success("Generating learner notes directly from your lesson plan... 🇬🇭", { duration: 3500 });

        generateNote(
          {
            ...loadedFormData,
            subject: currentDisplaySubject
          },
          { 
            school: profile?.school, 
            district: profile?.district,
            region: profile?.region,
            town: loadedFormData.specificLocality || profile?.town,
          ...
          if (user) {
            cacheGeneratedDocument({
              id: `note_${Date.now()}`,
              authorId: user.uid,
              title: data.title || `${currentDisplaySubject} - ${loadedFormData.subStrand || loadedFormData.strand || 'Lesson Notes'}`,
              type: 'note',
              subject: currentDisplaySubject,
              level: loadedFormData.level,
              class: loadedFormData.class,
          ...
          toast.error("Could not auto-generate notes. You can review parameters and click generate.");
          setStep(3);
        }).finally(() => {
          setLoading(false);
        });
  ...
  const handleGenerate = async () => {
    ...
    if (formData.indicator && formData.class) {
      const isIndicatorMatch = matchStandardToClass(formData.indicator, formData.class, formData.level);
      if (!isIndicatorMatch) {
  ...
  const handleDownloadPDF = () => {
    ...
    const drawPage1Header = () => {
      ...
      doc.text(splitMeta[0] || '', 105, 54, { align: 'center' });

      if (formData.contentStandard || formData.indicator) {
        const metadataLine3 = `CONTENT STANDARD: ${formData.contentStandard || 'N/A'} | INDICATOR: ${formData.indicator || 'N/A'}`;
        const splitMeta3 = doc.splitTextToSize(metadataLine3, 170);
        doc.text(splitMeta3[0] || '', 105, 58.5, { align: 'center' });
      }
    ...
    const renderParagraph = (textLine: string, x: number, isList: boolean, prefixStr: string) => {
      // 1. Parse markdown inline tags (bold, italic, code) while preserving words and punctuation
      const spanRegex = /(\*\*\*.*?\*\*\*|\*\*.*?\*\*|__.*?__|\*.*?\*|_.*?_|`.*?`|[^*_`]+)/g;
      const rawMatches: { text: string; bold: boolean; italic: boolean }[] = [];
      let m: RegExpExecArray | null;

      while ((m = spanRegex.exec(textLine)) !== null) {
        const raw = m[0];
        if (!raw) continue;
        if (raw.startsWith('***') && raw.endsWith('***') && raw.length > 6) {
          rawMatches.push({ text: raw.slice(3, -3), bold: true, italic: true });
        } else if ((raw.startsWith('**') && raw.endsWith('**') && raw.length > 4) ||
                   (raw.startsWith('__') && raw.endsWith('__') && raw.length > 4)) {
          rawMatches.push({ text: raw.slice(2, -2), bold: true, italic: false });
        } else if ((raw.startsWith('*') && raw.endsWith('*') && raw.length > 2) ||
                   (raw.startsWith('_') && raw.endsWith('_') && raw.length > 2)) {
          rawMatches.push({ text: raw.slice(1, -1), bold: false, italic: true });
        } else if (raw.startsWith('`') && raw.endsWith('`') && raw.length > 2) {
          rawMatches.push({ text: raw.slice(1, -1), bold: false, italic: false });
        } else {
          rawMatches.push({ text: raw, bold: false, italic: false });
        }
      }

      if (rawMatches.length === 0) return;

      // 2. Tokenize words while strictly tracking spacing between tokens
      ...
      let nextWordNeedsSpace = false;

      for (let sIdx = 0; sIdx < rawMatches.length; sIdx++) {
        const span = rawMatches[sIdx];
        const spanText = span.text;
        if (!spanText) continue;

        const startsWithSpace = /^\s/.test(spanText);
        if (startsWithSpace) {
          nextWordNeedsSpace = true;
        }

        const wordsInSpan = spanText.trim().split(/\s+/).filter(Boolean);

        for (let wIdx = 0; wIdx < wordsInSpan.length; wIdx++) {
          const w = wordsInSpan[wIdx];
          const hasSpace = (wIdx === 0 ? (formattedWords.length > 0 && nextWordNeedsSpace) : true);

          formattedWords.push({
            text: w,
            bold: span.bold,
            italic: span.italic,
            precededBySpace: hasSpace
          });
        }

        const endsWithSpace = /\s$/.test(spanText);
        nextWordNeedsSpace = endsWithSpace;
      }
      ...
      // 3. Line wrap word tokens
      const linesOfWords: FormattedWord[][] = [];
      let currentLine: FormattedWord[] = [];
      let currentLineWidth = 0;

      for (let k = 0; k < formattedWords.length; k++) {
        const fw = formattedWords[k];
        const isFirst = currentLine.length === 0;
        const wWidth = getWordWidth(fw, isFirst);

        if (!isFirst && currentLineWidth + wWidth > allowedWidth) {
          linesOfWords.push(currentLine);
          currentLine = [fw];
          currentLineWidth = getWordWidth(fw, true);
        } else {
          currentLine.push(fw);
      ...
      // 5. Render lines with styled text chunks preserving genuine spaces
      linesOfWords.forEach(lineWords => {
        ...
        let curChunk: TextChunk | null = null;

        lineWords.forEach((fw, wordIdx) => {
          const addSpace = (wordIdx > 0 && fw.precededBySpace);
          const wordStr = (addSpace ? ' ' : '') + fw.text;

          if (curChunk && curChunk.bold === fw.bold && curChunk.italic === fw.italic) {
            curChunk.text += wordStr;
          } else {
        ...
        chunks.forEach(chunk => {
          ...
          currentX += doc.getTextWidth(chunk.text);
        });
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      // Single neutral statement on Page 1 only
      if (i === 1) {
        doc.setFontSize(6.5);
